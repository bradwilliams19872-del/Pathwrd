#!/usr/bin/env python3
"""Insert batch 4 schools and profession-school links into mock-data.ts."""
...
# Insert new schools before the `];` that ends the schools array (right before `const professionSchools`)
old_schools_end = '  },\n];\n\nconst professionSchools'
new_schools_end = '  },\n' + new_schools + '];\n\nconst professionSchools'
content = content.replace(old_schools_end, new_schools_end)

# ---- new profession-school links ----
new_links = """  // Firefighter schools
...
# Insert new links before the `];` that ends the professionSchools array
old_links_end = '  { professionId: "p21", schoolId: "s32", programName: "Journalism (M.S. / M.A. — Columbia Journalism School)" },\n];'
new_links_end = '  { professionId: "p21", schoolId: "s32", programName: "Journalism (M.S. / M.A. — Columbia Journalism School)" },\n' + new_links + '];'
content = content.replace(old_links_end, new_links_end)

with open("src/lib/mock-data.ts", "w") as f:
    f.write(content)

print("Done! Inserted 7 new schools and 10 profession-school links.")
